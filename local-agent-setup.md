...
## Быстрый старт: весь путь одной последовательностью

Инструкция ниже дорабатывалась по ходу диалога (память, whitelist для скиллов, вторая модель и т.д.), и из-за точечных правок стало сложно проследить единый путь от пустой машины до работающей команды `agent`. Этот раздел — канонический порядок без развилок: там, где дальше в инструкции есть выбор (например, "Вариант А / Вариант Б"), здесь взят один вариант по умолчанию, а альтернативы остаются в соответствующем шаге как справка. Номера шагов в комментариях — ссылки на подробности ниже.

```bash
# Шаг 1 — модели (нужен интернет только здесь, один раз)
curl -fsSL https://ollama.com/install.sh | sh
ollama serve &                                            # или системный сервис, если уже настроен
ollama pull qwen2.5:14b-instruct-q4_K_M                    # модель-оркестратор
ollama pull qwen2.5-coder:14b-instruct-q4_K_M               # модель для кода (Шаг 6.5)
ollama pull nomic-embed-text                                # embedding-модель для RAG

# Шаг 2 — каркас проекта
mkdir -p ~/local-agent/{skills,docs,sessions,tmp,sandbox_images,scripts}
cd ~/local-agent
python3 -m venv .venv
source .venv/bin/activate
pip install ollama chromadb pyyaml docker typer rich

# Шаги 3–8 — код проекта (agent/*.py, skills/*)
# Создайте файлы agent/core.py, agent/skills.py, agent/skill_guard.py,
# agent/memory.py, agent/rag.py, agent/sandbox.py, agent/main.py
# и файлы конкретных скиллов — см. соответствующие шаги ниже, там даны
# полные листинги каждого файла.

# Шаг 9 — pyproject.toml строго в корне проекта, затем установка
cat > ~/local-agent/pyproject.toml << 'EOF'
[project]
name = "local-agent"
version = "0.1.0"
...
# Шаг 9, Вариант Б — глобальный вызов без активации venv (взят как вариант по умолчанию)
mkdir -p ~/.local/bin
ln -s ~/local-agent/.venv/bin/agent ~/.local/bin/agent
# добавьте в ~/.bashrc или ~/.zshrc, если ещё не сделано:
#   export PATH="$HOME/.local/bin:$PATH"

# Шаг 6 — образ браузера (нужен интернет один раз, дальше он единственный, кто его использует)
docker build -t agent-browser ./sandbox_images/browser

# Готово — команда доступна из любого терминала без активации venv:
agent "какой сейчас час"
```

Если что-то из этого не заработало — ищите подробности по номеру шага в скобках в комментариях выше: там объяснено, почему сделано именно так, и какие есть альтернативы (например, работа через активацию `.venv` вместо симлинка, или именованные сессии вместо одноразовых).
...
CREATE_SKILL_MANIFEST = {
    ...
    "description": (
        ...
        "делая новую возможность доступной для последующих вызовов. Перед регистрацией "
        "скилл автоматически проверяется: (1) статически на запрещённые импорты/вызовы, "
        "(2) функционально — реальным вызовом run(**test_call) с примером аргументов. "
        "Обязательно предоставляйте test_call с реалистичными значениями, иначе скилл "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "test_call": {
                "type": "object",
                "description": "Пример аргументов для run(**test_call) — используется для самопроверки перед регистрацией.",
            },
        },
        "required": ["skill_name", "description", "python_code", "test_call"],
    },
}

def create_skill(skill_name, description, python_code, test_call, parameters_schema=None, force=False):
    from .skill_guard import validate_skill_code
    if not force:
        validate_skill_code(python_code)  # бросит SkillCodeError, если что-то запрещено

    # Функциональная самопроверка: реально выполняем run(**test_call) во временном
    # модуле, не трогая диск, пока не убедимся, что скилл не падает с исключением.
    import importlib.util, sys, tempfile, pathlib as _pathlib

    with tempfile.TemporaryDirectory() as tmp:
        tmp_file = _pathlib.Path(tmp) / "candidate.py"
        tmp_file.write_text(python_code)
        spec = importlib.util.spec_from_file_location(f"_candidate_{skill_name}", tmp_file)
        module = importlib.util.module_from_spec(spec)
        try:
            spec.loader.exec_module(module)
            test_output = module.run(**test_call)
        except Exception as e:
            return (
                f"Скилл '{skill_name}' НЕ создан: тестовый вызов run(**{test_call}) "
                f"завершился ошибкой: {type(e).__name__}: {e}. "
                f"Исправьте код и попробуйте создать скилл заново."
            )
    ...
    return (
        f"Скилл '{skill_name}' создан и прошёл самопроверку. "
        f"Тестовый вызов вернул: {test_output!r}"
    )
```
...
**Про функциональную самопроверку.** `create_skill` теперь требует обязательный `test_call` — пример аргументов — и реально выполняет `run(**test_call)`, прежде чем записать скилл на диск. Это не просто пожелание в промпте ("проверь код перед регистрацией"), а структурное требование: без `test_call` вызов `create_skill` невалиден по схеме, а если тестовый вызов бросает исключение, скилл вообще не сохраняется, и модель получает текст ошибки — значит она физически не может зарегистрировать код, который не пытались хотя бы раз выполнить с примером данных. Важная оговорка: этот тестовый прогон выполняется **в процессе самого агента, без Docker-изоляции** — в отличие от `run_python`/`run_cpp` (Шаг 5). Это осознанный компромисс, а не недосмотр: к моменту теста код уже прошёл `validate_skill_code` (белый список импортов из Шага 3), то есть у него физически нет доступа к сети, файловой системе за пределами разрешённого и опасным вызовам — значит выполнить его напрямую безопасно. Если захотите ужесточить дальше, тестовый вызов тоже можно завернуть в `run_python` из песочницы — ценой чуть большей задержки на каждое создание скилла.
...
Сначала создайте `~/local-agent/pyproject.toml` — то есть **в корне проекта, рядом с папкой `agent/`, `skills/`, `docs/`** (не внутри `agent/`):
...
Только после этого — установка пакета в editable-режиме (порядок важен: `pip install -e .` ищет `pyproject.toml` в текущей директории):
...
### Bash-обёртки для часто используемых вызовов

Отдельные варианты вызова (продолжить сессию, зайти в конкретный проект, переиндексировать документы) удобно не набирать каждый раз руками, а сохранить в маленькие скрипты. Кладите их в `~/local-agent/scripts/` — это часть проекта, а не системная утилита, поэтому они не должны попадать в `~/.local/bin` вместе с самим `agent` (там — только сам исполняемый файл, см. Вариант Б выше).

```bash
mkdir -p ~/local-agent/scripts
```
...
`scripts/project.sh` (сессия под конкретную задачу, имя передаётся первым аргументом):
```bash
#!/usr/bin/env bash
name="$1"; shift
exec agent --session "$name" "$@"
```

`scripts/reindex_docs.sh` (переиндексация `docs/` после добавления новых файлов — короткий Python-вызов `ingest()` из Шага 4):
```bash
#!/usr/bin/env bash
source ~/local-agent/.venv/bin/activate
python -c "
from agent.rag import ingest
import pathlib
for f in pathlib.Path('docs').glob('*'):
    ingest(str(f))
...
```bash
chmod +x ~/local-agent/scripts/*.sh
```

Вызывать можно двумя способами:
```bash
~/local-agent/scripts/continue.sh "..."                 # по полному пути
bash ~/local-agent/scripts/project.sh diploma "..."      # через bash явно
```

Если хочется вызывать без пути (`continue.sh "..."` из любого места), симлинкуйте так же, как `agent`:
```bash
ln -s ~/local-agent/scripts/continue.sh ~/.local/bin/agent-continue
```
Тогда `agent-continue "..."` работает из любого терминала. Называйте такие симлинки с префиксом (`agent-...`), чтобы не путать их с системными командами и не засорять `PATH` неймингом без контекста.
...
- [ ] `agent "создай скилл с ошибкой в коде (например, обращением к несуществующей переменной) и попробуй его зарегистрировать"` — скилл не появляется в `skills/`, агент получает и показывает текст исключения из тестового вызова.
