...
## Simple Examples
+ Constants: `C = 1` -> `const double C = 1`. If there is no equations, the following error would be raised: "ValueError: No equation found"
+ Explicit time equation: `Dt(u) = 0` -> `u1[index] = u[index];`, `Dt(u) + DX(u) = 0` -> `u1[index] = u[index] + tau*((u[index + offset_X] - u[index - offset_X]) / (2*h_X));`. 
